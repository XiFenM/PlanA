...
| 每周学习进度／Lesson evidence | `计划/八周冲刺进度/W1.md` 至 `W8.md`，当前活动周由 Checkpoint 引用 | 只记录完成了什么、凭什么算完成、还缺哪些没完成；详细约定和证据用链接引用 |
| 详细验收与 Lesson Session event | `计划/八周冲刺进度/历史记录/` 下对应周的学习验收记录 | 保存已有练习约定、验收依据和实质阶段变化；不复制当前周进度、恢复游标或逐轮对话 |
...
技术成果保存在相应模块，面试成果保存在 `面试准备/自我准备/`；不再用技术设计包承载整周进度。W2–W8 的空白进度页只是用户要求的跟踪入口，不代表创建或启动新的 Lesson。旧设计包路径仅作历史链接兼容，不再作为新证据材料或状态 owner。
...
| 当前周完成判断、缺项与最终验收状态 | `计划/八周冲刺进度/` 下对应周进度文档 |
| 详细练习约定、历史 finding 与阶段变化 | 对应周的学习验收记录；周进度只引用其依据 |
