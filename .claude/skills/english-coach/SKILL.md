...
- **与 study-companion 共存**：英语轨学习或用户写英文时，english-coach 占轮末反馈位，陪学只驱动学习循环——**绝不两者同时追加反馈**（英语赢反馈、陪学赢仪式）。
...
1. 读当天的 `{module}/log/YYYY-MM-DD-*.md` 学习记录（没有就先跑 study-log skill 生成）；用 study-log 的脚本提取稿拿到用户当时的中文原话（scratchpad 里的 dump，过期就重新提取）。
...
1. **定位当天文件**：在 `英语/log/` 找编号最大的 `day-NN.md`，读其 H1 里的日期。
   - H1 日期 == 今天 → 追加进该文件。
   - 否则 → 复制 `英语/log/_template.md` 为新的 `day-(NN+1).md`，H1 设为 `# 学习日志 · Day NN — YYYY-MM-DD 周X`（今天的日期在 harness 上下文里），加一行 `来源：AI 教练对话（Claude Code）`。
