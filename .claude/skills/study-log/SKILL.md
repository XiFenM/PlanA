---
name: study-log
description: 从 Claude Code 原始对话记录（JSONL）提取技术学习过程，筛选后整理成结构化学习记录，写入 {module}/log/。产出是 memo-cards 技术制卡的过程素材（文章是结果素材，学习记录是过程素材）。Use when the user asks 整理学习记录 / 提取对话记录 / 把今天的学习对话存档.
---

# 学习记录提取（Claude Code 对话 → 结构化学习记录）
...
```bash
python3 .claude/skills/study-log/scripts/extract_transcript.py list
```

列出本项目全部会话（新→旧，含起止时间和首条用户消息）。默认目标 = 用户点名的日期/主题；没点名就取当天。

### 2. 提取对话（脚本）

```bash
python3 .claude/skills/study-log/scripts/extract_transcript.py extract \
  --session <id前缀> --date YYYY-MM-DD -o <scratchpad>/dump.md
```

脚本输出纯净的 用户/Claude 文本轮次（已剔除 thinking、工具调用、subagent 侧链、压缩摘要、system-reminder 注入）。跨天的长会话务必用 `--date` 切片；需要看改了哪些文件时加 `--tools`。
...
- 对话记录（`~/.claude/projects/…/*.jsonl`）**只读**；本 skill 只新建/覆盖 `{module}/log/` 下自己的文件。
