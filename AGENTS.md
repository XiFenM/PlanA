...
- `resource-planning` — the latest central core is selected, but PlanA does not attach a persistent context in this stage. It is limited to unconfigured, conversational, zero-write research. Do not save a brief or run refresh/review until source/query catalogs, portfolio slots, and the registry bootstrap are migrated and reviewed together.
