We are looking for a Software Test development engineer in NVIDIA’s AI SWQA team.  
我们正在寻找 NVIDIA AI SWQA 团队的软件测试开发工程师。  
The position is in NVIDIA AI Software Quality Assurance team that defines, develops and performs tests to validate robustness and measure the performance of NVIDIA‘s AI software and GPU Infrastructure for autonomous driving, healthcare, speech recognition, natural language processing, and a wide variety of other AI scenarios. This team collaborates with multiple AI product teams to develop new products; derive and improve complex test plans; and improve our workflow processes for a diverse range of GPU computing platforms. You should grow with being in the critical path supporting developers working for billion-dollar business lines as well as intimately understanding the values of responsiveness, thoroughness and teamwork. You should constantly foster and implement efficiency improvements across your domain. Join the team which is building software which will be used by the entire world!  
该职位属于 NVIDIA AI 软件质量保证团队，负责定义、开发并执行测试，以验证 NVIDIA AI 软件和 GPU 基础设施的鲁棒性并衡量其性能，这些软件和基础设施应用于自动驾驶、医疗保健、语音识别、自然语言处理以及各种其他 AI 场景。该团队与多个 AI 产品团队合作开发新产品；制定和改进复杂的测试计划；并改进适用于各种 GPU 计算平台的流程。您应随着在支持数十亿美元业务线开发者的关键路径上成长，同时深入理解响应性、全面性和团队合作的价值。您应在您的领域内不断培养和实施效率改进。加入这个正在开发将被全世界使用的软件的团队！  
 

**What you’ll be doing:  你将负责：**

- Work closely with global cross-functional teams to understand the test requirements and take ownership of product quality.  
    与全球跨职能团队紧密合作，理解测试需求并负责产品质量。
    
- Plan/design/execute/report/automate test plan/test case/test reports.  
    制定/设计/执行/报告/自动化测试计划/测试用例/测试报告。
    
- Manage bug lifecycle and co-work with inter-groups to drive for solutions.  
    管理缺陷生命周期，并与跨组合作推动解决方案。
    
- Automate test cases and assist in the architecture, crafting and implementing of test frameworks.  
    自动化测试用例，并协助架构设计、制作和实施测试框架。
    
- In-house repro and verify customer issues/fixes.  
    内部复现和验证客户问题/修复。
- Utilize AI-powered tools to improve efficiency and quality, including test case/plan/script generation, defect detection, CBTP, bug fixing and day to day assistance.    
    利用 AI 工具提高效率和品质，包括测试用例/计划/脚本生成、缺陷检测、CBTP、错误修复以及日常协助。    

**What we need to see:  
我们需要看到：**

- BS or higher degree in CS/EE/CE or equivalent.  
    计算机科学/电子工程/通信工程或同等学历的学士学位或以上。
    
- 5+ years of software quality assurance or test automation background with knowledge of test infrastructure and strong analysis skills.  
    5 年以上的软件质量保证或测试自动化背景，具备测试基础设施知识及强大的分析能力。
    
- Scripting language (Python, Bash) knowledge and UNIX/Linux experience.  
    脚本语言（Python、Bash）知识及 UNIX/Linux 经验。
    
- Good Python software development or test development experience.  
    具备良好的 Python 软件开发或测试开发经验。
    
- Good user/development experience of virtualization like VM & Docker container & k8s  
    良好的用户/开发体验，如虚拟化（VM、Docker 容器、k8s）
    
- Excellent English written and oral communication skills.  
    优秀的书面和口头沟通能力。
    
- Multiple GPUs P2P workload developing/testing  
    多 GPU P2P 工作负载的开发/测试
    
- Able to juggle conflicting/changing priorities and maintain a positive attitude while experiencing challenging and dynamic schedules.  
    能够应对冲突/变化的优先级，在挑战性和动态的日程安排中保持积极的态度。
    
- Proven success in leveraging AI (development) tools to significantly improve efficiency, streamline workflows, enhance process automation, create test cases and increase code coverage.  
    在利用 AI（开发）工具显著提高效率、简化工作流程、增强流程自动化、创建测试用例和增加代码覆盖率方面有成功的经验。
    
- Experience with LLM inference frameworks (TRT-LLM, vLLM, SGLang, etc.) and familiar with running various AI workloads  
    有 LLM 推理框架（TRT-LLM、vLLM、SGLang 等）的经验，熟悉运行各种 AI 工作负载
    
- Experience with AI tools for coding (like Cursor, Gemini, NotebookLM)  
    有用于编码的 AI 工具（如 Cursor、Gemini、NotebookLM）的经验
    

**Ways to stand out from the crowd:  
脱颖而出的方法：**

- Familiarity with NVIDIA GPU hardware products (Tesla, Tegra, DGX, etc.)  
    熟悉 NVIDIA GPU 硬件产品（Tesla、Tegra、DGX 等）
    
- Familiarity with multiple GPUs tools usage (NCCL / NIXL)  
    熟悉多 GPU 工具的使用（NCCL / NIXL）
    
- Understanding and working knowledge with any Deep Learning Framework especially in end-to-end customer scenarios.  
    了解并掌握任何深度学习框架，特别是在端到端客户场景中。
    
- Working knowledge of NVIDIA GPU Computing (CUDA) and CUDA libraries for Deep Learning like cuDNN  
    掌握 NVIDIA GPU 计算（CUDA）和深度学习 CUDA 库（如 cuDNN）
    
- Experience in VectorCAST, Bullseye, Gcov, or Coverity tools.  
    具备 VectorCAST、Bullseye、Gcov 或 Coverity 工具的使用经验。