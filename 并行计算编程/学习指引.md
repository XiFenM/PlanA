...
- **SIMT 模型映射**：在学习 Triton 时，时刻思考：Triton 的 `block` 对应 GPU 的 SM/CU，`warp` 对应什么？`shared memory` 对应什么？
- 在学习 Triton 时，**强制要求对照阅读 CUDA 编程指南**，理解 Triton 屏蔽掉的硬件细节（如 Bank Conflict, Memory Coalescing）在底层到底发生了什么。这也是 JD 中提到的 _"low-level programming"_。    
- **通用概念映射**：将你自研芯片的概念（如 DMA, Vector Unit, Local Memory）与 NVIDIA GPU 的概念（Copy Engine, Tensor Core, Shared Memory）建立映射表。面试时，用 GPU 的术语来解释你在自研芯片上的优化经验，这非常加分。
---
