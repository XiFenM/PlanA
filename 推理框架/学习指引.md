...
- 新增：**深入 Quantization (AWQ/GPTQ) 的 Triton 实现**。
	- **增加 W8A8 / W4A16 原理**：理解 Weight-Only Quantization (AWQ, GPTQ) 与 Activation Quantization (SmoothQuant) 的区别。
    
	- **增加 Kernel 实现**：学习如何在 Triton 中实现 INT8/FP16 的混合精度矩阵乘法（Mixed Precision MatMul）。
    
	- **工具链**：了解 **TensorRT-LLM** 中的量化流程（虽然你无法运行，但要理解其 Calibration 和 Layer Fusion 逻辑）。
- 新增：**Roofline Model 性能建模分析**（计算 vs 访存瓶颈分析）。
	- **Roofline Model**：必须掌握。学会画 Roofline Model 图，判断算子是 Compute-bound 还是 Memory-bound。
    
	- **Trace 分析**：学习看 **Nsight Systems** (NVIDIA) 或 **Rocprof** (AMD) 的 Trace 图。即使没有硬件，也要找一些开源的 Trace 文件来分析，识别 Pipeline Bubble、通信阻塞等典型图样。
    
	- **理论建模**：学会估算理论极限。例如，给定显存带宽和模型参数量，能手算出推理的极限 Token/s。
#### 2. 精选学习资料推荐
