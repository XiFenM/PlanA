...
配套 [学习指引.md §4](学习指引.md) 的"只收 4 类、每天 ≤5 张"原则，以及 [ai-chat-prompt.md](ai-chat-prompt.md) 里 AI 教练产出的 "Chunk of the day"。
...
```
英语/
├── 学习指引.md              # 稳定版计划（5 阶段 + 4 套 SOP + 卡片模板）
├── 进度.md                  # 进度追踪（打卡 + checkpoint）
...
`day-NN` 是学习日序号，一天一个文件；日期写在文件标题（`# 学习日志 · Day NN — YYYY-MM-DD`）。跳过没学的日子不占号。每周日在 [进度.md](进度.md) 的周打卡里汇总本周几个 day 文件。
...
> **"读 `英语/log/day-NN.md`，按 review-workflow Step 3 整理成墨墨表格，写到 `cards/day-NN.md`。"**
...
每周日把本周几天的复习收获汇总，在 [进度.md 周打卡](进度.md) 的"高光时刻"里记一句：**哪个曾经踩坑的点，这次脱口而出了。**
