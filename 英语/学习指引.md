...
>
> - 本文件（`学习指引.md`）= **稳定版**：整体设计、工具栈、每日时间块、4 套练习 SOP、Anki/墨墨卡片模板、分阶段详表、订阅源。月度 review 时才改。
> - 配套的 [`进度.md`](进度.md) = **进度追踪**：每日打卡、周/月数据对比、阶段 checkpoint、偷懒红线。每天填。
> - [`review-workflow.md`](review-workflow.md)：踩坑 → 墨墨记忆卡的 4 步复习流程。[`ai-chat-prompt.md`](ai-chat-prompt.md)：AI 技术陪练 + 英语教练 system prompt。
...
>
> - **周打卡**：每周日在 `进度.md` 的周追踪表填一行（美剧/口语课/AI Infra 听力/blog-voice 新增文章/mini-talk/Anki 数据 + 一个"听见的高光时刻" + 下周只改一件事）。
> - **月度对比**：每月末在 `进度.md` 的月度对比表记一行（听力基线均分 / 口语 2min 长停顿 / 阅读 wpm / Anki 总卡 / 最大进步点）。
...
### 7.3 多邻国的备份：Anki + 散时听 blog-voice 自产音频
