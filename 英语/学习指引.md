# 英语学习指引（听力 / 口语优先 · 22 周全程并行子轨道）

> **本模块在 PlanA 里的定位**：和 [Leetcode](../Leetcode/学习指引.md) 一样，是一条**全程并行子轨道**——不占主线板块的周次，而是每天用一个独立的 60–75 min 时间块推进，贯穿整个面试冲刺（并多跑 2 周到 W22，见 §0）。
>
> - 本文件（`学习指引.md`）= **稳定版**：整体设计、工具栈、每日时间块、4 套练习 SOP、墨墨复习策略、分阶段详表、订阅源。月度 review 时才改。
> - 配套的 [`进度.md`](进度.md) = **进度追踪**：每日打卡、周/月数据对比、阶段 checkpoint、偷懒红线。每天填。
> - [`review-workflow.md`](review-workflow.md)：结构化日志与墨墨暂存卡片的路径、授权和 Skill 交接。英语反馈行为由中央 `english-coach` 拥有。
> - **音频教材由外部工具仓库 `blog-voice` 生产**（见 §1.4 / 附录 A）。blog-voice 已回归"纯音频生成工具"定位，学习计划本身住在 PlanA；它和本仓库是**同级目录**（`../blog-voice`）。

## 0. 整体设计

### 我是谁、要解决什么

- 已工作的软件工程师，方向是 AI Infra（同级的 `blog-voice` 工具仓库就是为这件事服务的，专门产出定制听力教材）。
...
把测试结果保存在 `英语/baseline/week0/` 下，每月底重测一次同样的题目，对比改进。
...
材料：`../blog-voice/articles/pytorch-internals/source.txt` 的前 1200 词（或任一你想读的 AI Infra 博客）。
...
| 用途 | 工具 | 备注 |
| --- | --- | --- |
| 间隔重复 | **墨墨记忆卡** | 使用原创牌组；合格素材默认新增 8–12 张逻辑卡（软目标，不是硬上限） |
...
| 听写 | Whisper（blog-voice 已经能跑） | 用 fish.audio 或自带 ASR 做听写对照 |
...
> 以下命令在**同级的 `blog-voice` 工具仓库**里跑（`cd ../blog-voice`），它是这套计划的音频教材生成器，详见 [../blog-voice/README.md](../../blog-voice/README.md)。产出的 `merged.wav` + `subtitle.lrc` 就是你的散时听 / 精听 / shadowing 素材。

```bash
cd ../blog-voice    # 切到工具仓库
...
后续阶段 2 起，每 2–3 周在 `blog-voice` 里用 `blog-voice article add` 加一篇新 AI Infra 文章，**自产 transcript + audio**，这是这套计划最大的杠杆。
...
| 复盘 | 5 min | 复习墨墨到期卡；需要制卡时显式请求预览／保存 | 质量与软目标见 §4 |
...
- **blog-voice 自产的 AI Infra 音频**（`../blog-voice/articles/<slug>/merged.wav`）→ 熟悉技术语块发音
...
**"认识但没听出来"** 的词最值钱——需要强化时把已核验片段交给 `memo-cards` 预览墨墨卡片；不要因发现素材就自动写入。
...
| 留候选 | 60 s | 记下本集 1 个值得强化的表达；需要制卡时再显式交给 `memo-cards` |
...
模板、字段顺序、Markji 3.8+ 兼容面、逻辑身份、去重与发布由中央 `memo-cards` 统一管理；本仓库不再维护
第二份行为模板。默认先预览，用户明确要求保存时才写入 `英语/cards/`。完成准入、原子化和去重后，每个
学习日以新增 **8–12 张逻辑卡**为软目标：少于 8 张不凑数，所有 A 级卡不因超过 12 张而丢弃，强 B 级
超量时展示本批与可延后项。用户明确要求完整转换时，保留全部通过质量门槛的候选。

**禁忌**：

- ❌ `obscure = 模糊的`（孤立单词 + 中文翻译）
- ❌ 把 8–12 当硬上限、硬下限或自动制卡授权
...
- 墨墨总卡数以质量为先，阶段 1 建议控制在约 100 张以内
- AI Infra 听力暂不上手
- 散时听：`../blog-voice/articles/pytorch-internals/merged.wav` 重复 ≥ 3 轮
...
- **墨墨加入"AI Infra 技术语块"分类**：每周目标 ≥ 10 个合格新语块
...
- **Week 22 末重做 Week 0 三项基线测试**，所有数据归档到 `英语/baseline/week22/`，对比写一份 1 页总结
...
- [ ] 阅读基线 wpm 提升 ≥ 50%
- [ ] 墨墨总卡 ≥ 400 张，其中技术语块 ≥ 200 张
- [ ] 22 个 mini-talk 录音全部归档

---

## 6. 打卡 + 数据追踪 → 见 [进度.md](进度.md)

> 周打卡表、月度对比、阶段 checkpoint、偷懒红线全部搬到了配套的 [`进度.md`](进度.md)（PlanA 的"稳定版指引 / 进度"分离约定，和 Leetcode 一致）：
>
> - **周打卡**：每周日在 `进度.md` 的周追踪表填一行（美剧/口语课/AI Infra 听力/blog-voice 新增文章/mini-talk/墨墨数据 + 一个"听见的高光时刻" + 下周只改一件事）。
> - **月度对比**：每月末在 `进度.md` 的月度对比表记一行（听力基线均分 / 口语 2min 长停顿 / 阅读 wpm / 墨墨总卡 / 最大进步点）。
> - **阶段 checkpoint**：§5 各阶段末的 checkpoint 清单在 `进度.md` 里勾选。
> - **偷懒红线**：连续 3 天没碰英语 / 周打卡完成率 < 50% / 周日复盘录音连续 2 周没做——触发即下周降难度 + 砍量 25%。
>
> 每周日的英语打卡也顺手汇进 [计划/进度总表.md](../计划/进度总表.md) 的并行子轨道一栏，和 AI Infra 主线一起做 weekly rollup。
...
### 7.3 多邻国的备份：墨墨 + 散时听 blog-voice 自产音频

多邻国主要价值是**心理 streak**，不是真的提升。如果哪天觉得它在浪费时间，直接用"每天打开墨墨复习一次"代替即可——本质都是"维持每日英语接触感"。
...
## 附录 A：`blog-voice` 工具与英语训练的集成点

> 这些命令和路径都在**同级的 `blog-voice` 工具仓库**里（`cd ../blog-voice` 后执行）。blog-voice 已回归"纯音频生成工具"定位，本计划是它的下游消费者。

| 文件/命令（在 `../blog-voice/` 下） | 作用 |
| --- | --- |
...
**节奏建议**：每 2–3 周用 `blog-voice` 新产一篇 AI Infra 文章音频。22 周累计 8–10 篇，等价于自产了一个**完全为你听力定制的 AI Infra 教材**。这是任何课程都给不了的杠杆。
