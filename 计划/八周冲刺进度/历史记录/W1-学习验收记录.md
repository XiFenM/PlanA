...
> 当前周进度见 [W1](../W1.md)；本文件保存练习约定、详细验收依据和阶段变更历史，不承担当前进度或恢复游标。
> 迁入的旧记录保留截至 2026-09-15 的语境，后续阶段事件按各自日期理解。旧记录标题中的“当前”、相对周次及“本文件”均按当时记录理解；历史合同 JSON、revision 和 digest 不作改写，也不把当时记录的暂停或通过外推为现在的新授权。
> 迁移前完整版本可在 Git 提交 `54489e1` 的原路径查阅；原始对话、结构化学习日志与复习卡保持原样。
...
以下版本、环境及允许范围保留原学习阶段的快照，不描述当前机器，也不构成新的实践授权。
...
|---|---|---|---|
| vLLM | [`references/vllm`](../../../推理框架/references/vllm) | `1dc464d42681d22f38caf1fdc1eb632dc4421c45` | 浅克隆、detached HEAD、上游 `main` 比较快照；不替代教学基线 |
| SGLang | [`references/sglang`](../../../推理框架/references/sglang) | `78fa921189e3a66c7278733940c60a1e6fe6e467` | 浅克隆、detached HEAD、上游 `main` 比较快照；不替代教学基线 |
...
- Program 的计划预算与节奏只见[八周证据冲刺计划 §5](../../高级AI框架开发工程师-八周证据冲刺计划.md#5-时间预算与周节奏)。
- 实际学习工时只按真实归属写入对应模块 `进度.md`；本记录、周进度和学习断点均不复制工时。
...
- **知识成果**：[请求生命周期与输出处理](../../../推理框架/深入学习理解vLLM/2-Request-Lifecycle-and-Output.md)、[设备适配边界](../../../推理框架/深入学习理解vLLM/3-设备适配边界.md)；本记录保留学习条件及验收依据，不再指向独立适配工作稿。
- **最近结构化 evidence**：[Pass C-1 学习者流程图](../../../推理框架/深入学习理解vLLM/request_journey.drawio)。
- **Checkpoint 引用**：[唯一学习断点](../../学习断点.md)。
...
<a id="w1-p1-accepted-20260916"></a>

### 已接受练习：W1-P1 运行路径与输出契约

- **练习 ID／revision**：`plana-jd-w1-p1-20260916`／`1`。
- **digest**：`sha256:d5bcc1c317f2c10155cc47d5667a4a2662daf72e7f215016f15d8414e0b81987`。
- **接受事件**：用户在完整六块约定后明确回复“接受，开始P1”；对应 [本次接受记录](#plana-jd-w1-20260916-p1-start)，绑定本 revision 与 digest。
- **本次维度与来源**：复用已有概念证据，补配置路径的实践核对，以及输出契约的实现、事前预测和运行解释；不追溯改写旧 Lesson 的 required 维度，不写 final mastery。固定源码裁决实现，真实处理器运行只裁决本用例。
- **帮助边界**：导师维护辅助工具、验收与环境，学习者保留核心实现和解释；实质提示只影响对应范围，使用一个无提示小变式恢复。当前未代写学习者核心文件。
- **非目标与完成门槛**：A1–A4 及映射到它们的阻塞缺项通过后收口 P1。只证明前端过滤、文本判停与取消列表；HTTP／ZMQ、Core 执行取消、GPU KV 回收、模型推理和性能均未覆盖。P2 与 W1 最终验收分别处理。
- **P1 验收结论（2026-09-17）**：按本约定 A1–A4 通过，练习收口。开放 blocking／major 为 0；`p1-F4` 原条件与证据分层缺口已解决，仅保留一个非阻塞的表格来源路径笔误。保留步骤指导与补差范围，不写 W1／Lesson final mastery，不自动启动 P2。
- **规范化契约**：digest 仅覆盖下列字段；递归按 key 排序、UTF-8、JSON 紧凑分隔符。

```json
{
  "id": "plana-jd-w1-p1-20260916",
  "targets": [{"objective_id":"w1-o1-repository-process-map","missing_dimensions":["practical"],"evidence_gap":"已有组件概念验收，缺少明确配置、源码依据与验证状态的路径表。"},{"objective_id":"w1-o2-request-lifecycle","missing_dimensions":["practical","empirical"],"evidence_gap":"已有输出与判停概念验收，缺少学习者核心场景实现、事前预测和真实 OutputProcessor 运行解释。"}],
  "task": "固定 vLLM v0.26.0@568afb3a13806beb53bb2e6bd518269357b237c0，复用已核验 GPU 环境及 Qwen tokenizer。路径核对采用单卡、DP=TP=PP=1、Executor uni、eager 模式。学习者提交配置到 Worker/Runner/Attention backend/相关 fallback 的源码表，并实现单请求 FINAL_ONLY、stop string 跨两次模拟 EngineCore 输出且 Core 未自行判停的场景；导师准备辅助与验收工具。",
  "deliverables": [{"artifact":"推理框架/实践/W1-P1/scenario.py","outcome":"学习者实现调用真实 OutputProcessor 的核心场景。"},{"artifact":"推理框架/实践/W1-P1/分析.md","outcome":"学习者提交路径表、事前预测、结果解释与证明边界；先在对话中进行分步检查，最终由学习者整理到文件。"}],
  "acceptance": [{"id":"A1","criterion":"路径表有固定源码依据，区分 Executor 与 Attention backend，说明相关 fallback 条件及源码推导、配置选择、实际模型执行的验证状态。","evidence_method":"分析.md 的源码与配置证据 Review。"},{"id":"A2","criterion":"单请求 FINAL_ONLY；stop string 跨两次模拟 EngineCore 输出出现，Core 均未自行判停。","evidence_method":"学习者 scenario.py 调用真实 OutputProcessor；test_contract.py 检查请求参数和实际输入。"},{"id":"A3","criterion":"检查中间输出、最终文本与结束原因、取消列表、前端请求状态，以及迟到输出是否重复交付。","evidence_method":"harness.py 记录真实处理器调用与状态，test_contract.py 验收，results.json 保存核验结果。"},{"id":"A4","criterion":"运行前给出预测，运行后解释结果与证明边界，再独立完成一个小变式。","evidence_method":"先在对话或分析.md 提交预测；运行后 Review 解释，再用一个小变式复核。"}],
  "scope": {"learner_owned":[{"artifact":"推理框架/实践/W1-P1/scenario.py","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P1/分析.md","operations":["create","modify"]}],"agent_owned":[{"artifact":"推理框架/实践/W1-P1/harness.py","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P1/test_contract.py","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P1/results.json","operations":["create","modify","record"]},{"artifact":"计划/八周冲刺进度/W1.md","operations":["read","modify","record"]},{"artifact":"计划/八周冲刺进度/历史记录/W1-学习验收记录.md","operations":["read","modify","record"]},{"artifact":"计划/学习断点.md","operations":["read","modify","record"]}],"read_only":[{"artifact":"../plana-w1-env/vllm-v0.26.0","operations":["read"]},{"artifact":"../plana-w1-env/environment.json","operations":["read"]},{"artifact":"../plana-w1-env/hf-cache/models--Qwen--Qwen3-0.6B/snapshots/c1899de289a04d12100db370d81485cdf75e47ca","operations":["read"]},{"artifact":"推理框架/深入学习理解vLLM","operations":["read"]}],"excluded":[{"artifact":"新增依赖、下载模型权重、修改框架源码、完整模型推理与性能测试、HTTP/ZMQ 时序和 Core 实际取消及 GPU KV 回收验证","operations":[]},{"artifact":"改写学习者核心工件、自动关闭 W1 或启动 P2/W2、自动生成文章/日志/卡片、Git 暂存/提交/推送","operations":[]}]},
  "optional": []
}
```
...
- [辅助工具](../../../推理框架/实践/W1-P1/harness.py)、[验收检查](../../../推理框架/实践/W1-P1/test_contract.py)、[运行证据](../../../推理框架/实践/W1-P1/results.json)由导师维护；学习者已提交 [scenario.py](../../../推理框架/实践/W1-P1/scenario.py) 与 [分析.md](../../../推理框架/实践/W1-P1/分析.md)。
- 本机环境复用 [版本与检查记录](../../../../plana-w1-env/environment.json)：RTX 5090、PyTorch `2.11.0+cu130`、vLLM `0.26.0`；已安装 wheel 构建提交为 `gffd46bfab`，固定教学树的 2034 个 Python 文件逐字节一致，不外推原生二进制来源一致。
- 2026-09-16 导师初始工具检查：普通 LENGTH 结束基线通过；当时目标 A2／A3 因学习者文件尚未提交而跳过，结果为 `1 passed, 2 skipped`。该历史结果保留在 `results.json` 的 `baseline_pytest`，不作为实际目标用例通过证据，也不使用人为 expected red。
- 配置构造实测为 `uni`、`use_v2_model_runner=true`、BF16、head size 128，Attention backend 尚为自动选择；Worker／Runner 未实例化、Attention selector／kernel 未执行。该新配置不覆盖旧 MRV1 题设的历史证据。
- 2026-09-16 [CUDA 选择器实测](../../../推理框架/实践/W1-P1/results.json)：在普通 Qwen3 Attention、SM12、BF16、head size 128、KV dtype `auto`、未显式指定 block size／backend 的条件下，`FLASH_ATTN`、`FLASHINFER`、`TRITON_ATTN`、`FLEX_ATTENTION` 通过校验；`TURBOQUANT` 因 `kv_cache_dtype not supported` 被排除。最终返回 `vllm.v1.attention.backends.flash_attn.FlashAttentionBackend`。这证明候选校验与自动选择，不证明 Attention kernel、Worker 或 Runner 已执行。
- 2026-09-16 学习者主用例实测：导师修复验收加载器缺少包上下文的问题后，原始 `scenario.py` 通过全部三项检查（`3 passed`）。A2／A3 在 `include_stop_str_in_output=False` 条件下通过；每次输出数量为 0／1／0，最终文本为 `The river `，仅第二次返回内部取消 ID，第二次起前端状态已删除。具体代码哈希、断言证据与加载器修复范围见 `results.json`；未改写学习者代码。
- 2026-09-16 运行后变式：学习者事前正确预测仅最终文本增加 stop string，数量、取消列表和清理时机不变。通过 `P1_INCLUDE_STOP=1` 创建变式输入，同一份 `scenario.py` 再次 `3 passed`，最终文本为 `The river is quiet`。前后代码哈希一致，未修改学习者实现；证据见 `results.json` 的 `include_stop_variant`。
- 2026-09-17 分析稿终审：教学版本、四个类名、本次配置条件及各项证据层级均已明确；预测修正、参数传递及证明边界解释可接受。`p1-F4` 原 major 缺口已解决，仅剩表格 Worker 来源路径的 minor 笔误，正文第 35 行及方法名已准确，不阻塞 A1。代码和工具哈希未变化，复用两组 `3 passed`；[A1–A4 验收结论](../../../推理框架/实践/W1-P1/results.json)已记录，P1 通过。
- 2026-09-18 定位修正复核：学习者将表格 Worker 来源改为 `vllm/platforms/cuda.py` 中的 `CudaPlatformBase.check_and_update_config()`，与正文和固定源码一致；`p1-F4` 关闭。只更新当前分析稿哈希与备注，保留验收时的原稿哈希；未重跑测试、改写学习者文件或刷新已归档日志／卡片。

**P1 当前 Review：预测补差与分析稿**

- 首次预测保留在 [results.json 的 learner_prediction](../../../推理框架/实践/W1-P1/results.json)，不以纠正后的预期覆盖原始摘要。文字中首次／迟到输出均称“无输出”，表格数量却填写 1／2；第二次保留了应排除的 stop string，并认为前端状态等待 Core abort 完成才删除。
- 下表的关闭依据为固定源码及 [纸面变式回答](../../../推理框架/实践/W1-P1/results.json)，只表示预测理解差距已修复；后续目标用例的本地执行证据单独见上方主用例实测，不改写原预测或旧口头练习结论。

| ID | 映射 | 严重度 | owner | 状态 | Evidence | 下一动作 |
|---|---|---|---|---|---|---|
| `p1-F1` | A3、A4 | major | learner | closed | 首次预测数量 1／2／2 与“无输出”文字矛盾；2026-09-16 变式中正确给出第二次 1 个、第三次 0 个及没有取消项。导师补准列表本身与 `len()` 数量的表述、空列表为 `[]`。 | — |
| `p1-F2` | A3、A4 | major | learner | closed | 首次在排除 stop 时仍保留 `is quiet`；变式改为 stop=`END`、保留 stop，学习者正确得到 `Go END`，未保留后续 ` now.`。 | — |
| `p1-F3` | A3、A4 | major | learner | closed | 首次认为前端状态等待 Core abort；变式明确 Core 尚未取消，学习者仍正确判断第二次状态已删除、第三次无输出与取消项，并解释因查不到前端状态而跳过。 | — |
| `p1-F4` | A1 | minor | learner | closed | 原条件与证据分层缺口于 2026-09-17 修复；2026-09-18 学习者进一步修正表格 Worker 来源路径，方法名、表格文件位置与正文均指向 `vllm/platforms/cuda.py` 的配置选择函数。定位复核通过。 | — |

- **非阻塞表达建议**：2026-09-17 已采纳：删除由 TODO 推断 external launcher 未实现的表述，补全测试入口读取 `P1_INCLUDE_STOP`、经 case 传给 SamplingParams 的链路。这两项不再保留待办。

- **帮助与独立证据**：导师针对原题直接给出正确预期、列表口径、文本截断和前端清理顺序，保留该帮助事实；没有代写核心实现。学习者随后在新文本、保留 stop 且 Core 尚未取消的纸面变式中通过复核，恢复对应理解范围的证据；该复核不代替正式实现、运行与运行后小变式。
- **实现阶段帮助**：学习者创建 `scenario.py` 骨架后主动表示不清楚 `run_case` 的逻辑。导师补充参数构造、请求对象与注册的区别、同一处理器上的三次消息提交，以及观察器和验收工具的分工。这是 A2 调用组织上的步骤指导，后续不将对应实现描述为完全无提示完成；独立证据由既定运行后小变式复核，未代写学习者文件，不重开已经通过的纸面理解检查。
- **变式帮助边界**：变式行为预测独立正确；学习者认为必须在函数内强制指定 True，导师据现有代码补准 `P1_INCLUDE_STOP → case → SamplingParams` 的参数传递，同一函数可接受两种输入。保留这部分提示事实，分析稿需由学习者解释该关系；没有将硬编码写入学习者实现。

<a id="w1-p2-accepted-20260917"></a>

### 已接受练习：W1-P2 共享库加载错误定位与修复

- **练习 ID／revision**：`plana-jd-w1-p2-20260917`／`1`。
- **digest**：`sha256:6f073ac71a671e72c01706f8e8ad7743b998a8ba092c5b87fd12b3edf5b2fef5`。
- **接受事件**：用户在六块约定后明确回复“确认接受”；见 [P2 启动记录](#plana-jd-w1-20260917-p2-start)，绑定本 revision 与 digest。
- **目标与范围**：复用 C++／Linux 既有概念验收，只补 ELF／动态库操作证据；学习者负责诊断、启动入口修复和解释，导师准备自建 C++17 fixture、构建、进程控制与验收。独立实践与本地运行 evidence 只覆盖本契约，不改变 vLLM Lesson 的历史 required 维度。
- **运行方法**：两份 `libw1probe.so` 同名且 SONAME 相同，app 构建时链接 lib-a；运行进程使用指定库搜索环境。本实验启用函数符号延迟绑定，app 打印 READY 并等待 `call`，便于在故障函数调用前检查目标 PID 的 maps。导师工具控制该阶段，不要求学习者猜测实验方法参数。
- **来源权威**：本机编译与运行以 [results.json](../../../推理框架/实践/W1-P2/results.json) 的版本、命令和工件哈希为锚点；装载规则参考 [ld.so(8)](https://man7.org/linux/man-pages/man8/ld.so.8.html)，环境依赖查询参考 [ldd(1)](https://man7.org/linux/man-pages/man1/ldd.1.html)，链接选项参考 [GNU ld](https://sourceware.org/binutils/docs/ld/Options.html)。在线说明不替代本机操作证据。
- **帮助与边界**：允许自然求助；导师准备方法与工具，学习者保留启动脚本和解释。实质提示只影响对应范围，用既定小变式复核。只编译、检查、运行自建工件，设置仅作用于实验进程；不修改系统库路径配置或真实业务库。P2 完成后另行汇总 W1 最终验收，不自动关闭 W1。
- **P2 验收结论（2026-09-18）**：分析稿 Review 通过，B1–B4 的既定要求满足，练习收口。开放 blocking／major 为 0；只裁决自建程序的 ELF／共享库排查及 A／B 两种父搜索路径，不扩展为完整 ABI、生命周期、并发或真实服务能力。W1 最终关闭另待用户确认。
- **规范化契约**：digest 仅覆盖下列字段；递归按 key 排序、UTF-8、JSON 紧凑分隔符。

```json
{
  "id": "plana-jd-w1-p2-20260917",
  "targets": [{"objective_id":"w1-p2-cpp-linux-validation","missing_dimensions":["practical","empirical"],"evidence_gap":"C++／Linux 基线概念已验收，仍缺同名共享库加载错误的独立诊断、启动入口修复及目标函数调用证据。"}],
  "task": "选择 ELF／动态库这一最低充分部分。导师准备调用 compute(int) 的 C++17 小程序与两份同名共享库，构造运行时加载错误版本而缺符号的可重复故障。学习者查看证据、定位原因，编写 launch.sh 修正实验启动环境，记录预测、诊断及验证结论。",
  "deliverables": [{"artifact":"推理框架/实践/W1-P2/launch.sh","outcome":"学习者编写实验启动入口，使新进程加载预期库并实际完成 compute(int) 调用。"},{"artifact":"推理框架/实践/W1-P2/分析.md","outcome":"学习者记录运行前预测、ELF/符号/查询环境/目标 PID 映射证据、修复依据、运行与小变式解释及证明边界；预测可以先在对话提交。"}],
  "acceptance": [{"id":"B1","criterion":"实际运行前给出结果预测和机制依据。","evidence_method":"学习者诊断或修复用例运行前的对话／分析.md；导师 fixture 预核验不冒充学习者 evidence。"},{"id":"B2","criterion":"用 ELF 依赖、动态符号、当前环境解析和目标 PID 映射说明程序实际加载的库及缺失符号。","evidence_method":"对自建工件的 readelf/nm/ldd 和目标 /proc/PID/maps 操作证据及解释 Review。"},{"id":"B3","criterion":"修正启动入口后，新进程映射到预期库，且真正调用 compute(int) 得到正确结果。","evidence_method":"test_contract.py 校验目标进程映射、函数结果与退出状态；harness.py 维护构建及运行证据。"},{"id":"B4","criterion":"解释证据边界，并在改变父进程库搜索环境后，对同一启动入口完成一次小变式复核。","evidence_method":"先说明变式预期，再运行启动入口检查；分析.md 或对话解释输入环境、目标进程映射与调用结果。"}],
  "scope": {"learner_owned":[{"artifact":"推理框架/实践/W1-P2/launch.sh","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/分析.md","operations":["create","modify"]}],"agent_owned":[{"artifact":"推理框架/实践/W1-P2/fixture/api.hpp","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/fixture/library.cpp","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/fixture/driver.cpp","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/harness.py","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/test_contract.py","operations":["create","modify","run"]},{"artifact":"推理框架/实践/W1-P2/results.json","operations":["create","modify","record"]},{"artifact":"../plana-w1-env/p2-build/app","operations":["create","modify","run"]},{"artifact":"../plana-w1-env/p2-build/lib-a/libw1probe.so","operations":["create","modify","run"]},{"artifact":"../plana-w1-env/p2-build/lib-b/libw1probe.so","operations":["create","modify","run"]},{"artifact":"计划/八周冲刺进度/W1.md","operations":["read","modify","record"]},{"artifact":"计划/八周冲刺进度/历史记录/W1-学习验收记录.md","operations":["read","modify","record"]},{"artifact":"计划/学习断点.md","operations":["read","modify","record"]}],"read_only":[{"artifact":"推理框架/实践/W1-P1","operations":["read"]},{"artifact":"参考源码、系统配置与系统库","operations":["read"]},{"artifact":"/proc/<本次实验 app 的 PID>/maps","operations":["read"]}],"excluded":[{"artifact":"修改系统级库搜索配置、替换真实业务库、运行真实服务、读取无关进程或对不可信二进制执行 ldd","operations":[]},{"artifact":"新增依赖、完整 ABI／并发／性能／RAII 或 sanitizer 专项、自动关闭 W1 或启动 W2、自动生成文章日志卡片及 Git 暂存提交推送","operations":[]}]},
  "optional": []
}
```
...
- 导师维护 [fixture](../../../推理框架/实践/W1-P2/fixture/driver.cpp)、[辅助工具](../../../推理框架/实践/W1-P2/harness.py)、[验收检查](../../../推理框架/实践/W1-P2/test_contract.py)与[运行证据](../../../推理框架/实践/W1-P2/results.json)。学习者已提交 [launch.sh](../../../推理框架/实践/W1-P2/launch.sh) 与 [分析.md](../../../推理框架/实践/W1-P2/分析.md)。
- 编译产物位于 `../plana-w1-env/p2-build/`，相对 PlanA 根目录；只生成 app 与 lib-a/lib-b 中两份自建库。实际 OS、编译器、binutils、标准库定位与产物哈希保存在 results.json；不沿用 P1 的旧机器环境快照。
- 2026-09-17 导师预核验 `2 passed, 1 skipped`：ELF／符号与当前环境查询检查通过；成功调用和可重复缺符号故障已由导师确认。跳过仅因学习者启动脚本不存在，不表示 B3 通过。
- 该预核验只证明实验工具可靠，不计为学习者 B1／B2／B3／B4 evidence。学习者随后实际完成预测、readelf／nm、ldd 与目标 PID 映射及故障调用：app 需要 `compute(int)`，A 环境的 ldd 解析到 lib-a，PID 102268 实际映射 lib-b；到达 READY 后首次调用报 `_Z7computei` 缺失，退出码 127。学习者正确说明两个进程的环境不同，因此解析／映射不矛盾。`c++filt` 确认错误符号对应 `compute(int)`；B1／B2 操作与解释通过，原始摘要与实际记录分别保留于 `learner_binary_evidence`、`diagnostic_observation`、`learner_diagnosis_review`。
- 2026-09-17 B3 通过：学习者提交启动脚本，并留下 `learner_run_parent_B` 成功验收记录。导师核对脚本及 app 哈希一致；父搜索路径为 B 时，新 PID 109803 实际映射 lib-a，目标调用输出 `RESULT 14`、退出码 0、stderr 为空。未修改学习者脚本，也未重复运行已有通过用例。
- 2026-09-17 B4 运行变式通过：学习者事前正确预测 A 输入不进入 if、不执行 export，app 仍使用 lib-a 并返回 14。导师补充环境继承说明后，只运行该 A 用例（1 passed）；目标映射、调用输出与退出状态符合预测，脚本哈希与 B 用例相同。
- 2026-09-18 分析稿终审通过：工具版本、符号与装载证据、修复逻辑及变式解释一致，明确仅覆盖父库路径恰为 A／B 的自建程序。脚本、工具及二进制哈希未变，复用现有通过证据；B1–B4 完成，无学习者 blocking／major finding，详见 results.json 的 `practice_acceptance`。

<a id="w1-final-gate-proposal-20260918"></a>

### W1 最终验收提议（2026-09-18，待用户确认）

P1／P2 已按各自接受的契约完成。本节将早期未显式登记的目标维度按既有任务和已接受实践投影为待确认边界，不增加新任务，不回写旧契约；尚未写入 `final_mastery`。

| 目标 × 本次拟 required 维度 | 最小证据与适用范围 | 判断 |
|---|---|---|
| `w1-o1-repository-process-map` × 概念 | Pass A–B、独立文字表达与进程／组件边界检查 | 充分 |
| `w1-o1-repository-process-map` × 实践 | P1 的实际配置、源码路径表与选择器核验；不外推完整进程启动 | 充分 |
| `w1-o2-request-lifecycle` × 概念 | Pass C、请求流程图、主链复述与边界变式 | 充分 |
| `w1-o2-request-lifecycle` × 实践 | 学习者 scenario.py 主用例和变式通过；保留实现步骤指导与参数提示事实 | 充分（约定范围） |
| `w1-o2-request-lifecycle` × 实证 | 模拟 EngineCore 输出驱动真实 OutputProcessor，两组运行、预测修正与解释可追溯 | 充分（前端输出契约） |
| `w1-o3-extension-boundaries` × 概念 | Pass D–E 与设备适配矩阵，能够辨别职责和改造边界 | 充分 |

- **不要求的维度**：`w1-o1` 不另要求完整进程／GPU 运行实证；`w1-o3` 的实际后端适配与设备实证为本周 not-required。上述划分待用户与 W1 关闭一并确认，不将未做的设备工作写成已掌握。
- **其他 W1 既定任务**：C++／Linux 概念基线与 P2 所选 ELF／动态库实操通过；19 条 Claim 审计、Conv3D Case Card v1 草稿与反压补充沿用已有验收。P2 不清零其他 C++／并发／完整 ABI 的历史能力缺口；Conv3D 的模型级精度等未覆盖项仍保留原边界。
- **阻塞与帮助**：开放 blocking／major 为 0。P1 原预测差距经纸面变式修复，主用例和运行后变式通过；步骤指导与参数提示如实保留，不宣称完全无提示独立实现。P2 的方法与 C++ fixture 由导师提供，学习者完成诊断、启动脚本及解释。
- **非阻塞定位备注**：P1 分析表 Worker 来源路径笔误已于 2026-09-18 由学习者修正并复核，`p1-F4` 关闭；不改变既有通过结论，不要求重测。
- **关闭建议**：按以上维度和证据边界关闭 W1，正式口头 Mock 沿既定 W4 及后续安排。用户确认前仅置于 `mastery-gate`；不自动启动 W2，不自动返回冻结主计划，不记录未提供的工时。

<a id="w1-cpp-linux-baseline-20260914"></a>

### W1 补充基线：C++／Linux 概念与书面校准

- **确认日期与结论**：2026-09-14，用户确认“C++／Linux 基线概念与书面校准完成，实践待验证”。本节对应[冲刺计划 W1 的基线任务](../../高级AI框架开发工程师-八周证据冲刺计划.md#62-主任务)，不新增 Lesson，也不关闭 W1 或覆盖历史诊断。
...
| 编译、sanitizer、ELF／动态库实测与服务启动配置 | 🟨 实践待验证 | 仅阅读示意代码、命令和模拟证据。环境变量继承、配置落实到真实服务启动入口、实际触发原故障调用由导师补充，未完成独立实操验证；见 [W1-P2](../W1.md#w1-p2-cpp-linux-validation)。 |
...
[Claim-evidence 矩阵](../../../面试准备/自我准备/Claim-evidence矩阵.md)与[Conv3D Case Card](../../../面试准备/自我准备/Conv3D-Case-Card.md)独立维护；本文件只保留它们的学习验收依据。
...
- **当前 Review 状态**：A1–A6 在已接受 revision 1 的草稿范围内通过，正式练习已结束；[Case Card v1](../../../面试准备/自我准备/Conv3D-Case-Card.md#project1-conv3d-case-card-v1)由学习者对话核心内容忠实整理后保存。图片性能的 10 例均值、视频单次测量、未覆盖的视频模型级精度及原始资料缺口均明确保留；未确认可披露的数值没有写入项目正文。项目经历继续按用户口述／自核层使用，不升级为原始 commit、Issue、脚本或运行结果的独立审计，不写 Lesson final mastery。
...
<a id="session-event-索引"></a>

## 学习阶段变更记录

这里回答“哪一段学习让证据或待办发生了什么变化”，不是逐轮对话日志。下列内容按原顺序保留；后续只在实质验收、约定或范围变化时追加简短依据，不复制当前周的完成清单、恢复游标或全部产物。
...
- **Evidence 引用**：[结果文章](../../../推理框架/深入学习理解vLLM/1-Repository-and-Process-Architecture.md)、[结构化过程记录](../../../推理框架/log/2026-08-09-vllm-repository-and-process-architecture.md)、[技术记忆卡](../../../推理框架/cards/vllm-repository-and-process-architecture.md)。
...
- **Evidence 引用**：[学习者 Draw.io 流程图](../../../推理框架/深入学习理解vLLM/request_journey.drawio)、[导出 PNG](../../../推理框架/深入学习理解vLLM/request_journey.png)。
...
- **已完成动作**：通过 `STOP / FINISHED_ABORTED / PREEMPTED` 对照与全链路复述；经补正区分 IPC 解码与内部 Request 构造，并通过跨 step 变式说明计算载荷与结束通知可以共存、最终输出不等待设备侧状态清理。补充明确普通结束时 KV blocks 已由 Scheduler 归还；导师完成 [12 文件源码索引](../../../推理框架/深入学习理解vLLM/2-Request-Lifecycle-and-Output.md#pass-c-source-map)。
...
- **Evidence 引用**：[控制面／执行面契约](../../../推理框架/深入学习理解vLLM/2-Request-Lifecycle-and-Output.md#pass-d-execution-contract)。
...
- **综合表达 evidence**：学习者正确对应 Worker／Runner、KV 布局、OOT 算子与 communicator 接入点，并指出题设差异不构成修改 EngineCore／Scheduler 的充分理由；识别私自回收仍 RUNNING 请求 KV 的契约破坏。导师补齐算子数学／tensor 契约及完整抢占生命周期，整理为 [六层适配矩阵](../../../推理框架/深入学习理解vLLM/3-设备适配边界.md#pass-e-adaptation-matrix)。
...
- **Evidence 引用**：[按本轮授权保存的结构化学习记录](../../../推理框架/log/2026-09-07-kv-padding-block-size.md)。插件快照与同事转述只支持该记录标明的范围，不替代课程固定源码或设备实验。
...
- **已完成动作**：按用户指令，将早期技术周的口头验收移交[冲刺计划 §5.3](../../高级AI框架开发工程师-八周证据冲刺计划.md#53-每周统一验收门)与 W4 及后续 Mock；保留已完成文字复述、追问与源码证据，旧练习 revision 1 的规范化内容未变。
...
- **已完成动作**：在既有验证包中登记 [W1-P1](../W1.md#w1-p1-runtime-output-validation) 的范围、候选验证点、材料与启动边界；更新恢复指针，当前不自动进入实践。
...
- **已完成动作**：完成三个书面综合情境及必要局部补差；用户确认保存基线结论和待实践项。记录 [基线缺口表](#w1-cpp-linux-baseline-20260914)，登记 [W1-P2](../W1.md#w1-p2-cpp-linux-validation)。
...
- **已完成动作**：完成 [19 条当前 Claim 矩阵](../../../面试准备/自我准备/Claim-evidence矩阵.md#w1-claim-evidence-matrix-20260914)；裁决为已证明 2 条、需降级 14 条、待补证 3 条，每条都有当前可用口径或关闭条件。
...
- **已完成动作**：学习者完成 F1–F6 核心内容和 A6 独立变式，A1–A6 按既定草稿范围通过；忠实整理并保存 [Case Card v1](../../../面试准备/自我准备/Conv3D-Case-Card.md#project1-conv3d-case-card-v1)，完成 W1 对应的 Case Card 验收项。
...
- **结构化记录**：[C++／Linux 基线](../../../推理框架/log/2026-09-14-cpp-linux-baseline.md)、[Conv3D 案例证据与反压](../../../推理框架/log/2026-09-15-conv3d-claim-backpressure.md)。覆盖选定的 210 条可见消息；使用稳定消息 ID 回溯来源，不把导师补充、用户澄清或课堂模拟改写为用户错误或实操证据。
- **复习资料**：[C++／Linux](../../../推理框架/cards/w1-cpp-linux-baseline.md) 11 张、[Conv3D 证据与反压](../../../推理框架/cards/conv3d-evidence-backpressure.md) 12 张；对应模板 XLSX 与受管清单均由制卡工具生成。最终 Review 将两份日志中的措辞补准／导师问法纠正改标为“要点”；用户确认后更新卡片来源哈希，问答、ID 和 XLSX 均未改变。两组同请求复核均得到 `operation=no-op`、`would_write=false`，无来源依赖或表格漂移。排除 3 项与旧卡语义重复的候选；只保留可公开的通用机制与证据边界，不制入未确认可披露的项目参数、性能数字或待实践项。未上传墨墨。
...
|---|---|---:|---|
| [vLLM 全会话复盘](../../../推理框架/log/2026-09-08-vllm-full-session-review.md) | [同名原文](../../../推理框架/log-raw/2026-09-08-vllm-full-session-review.md) | 324 | `sl-0eec17119ac5482d9e252bca982279ac` |
| [C++／Linux 基线](../../../推理框架/log/2026-09-14-cpp-linux-baseline.md) | [同名原文](../../../推理框架/log-raw/2026-09-14-cpp-linux-baseline.md) | 90 | `sl-df9352532bc9402a9d8e03244e68c95f` |
| [Conv3D 证据与反压](../../../推理框架/log/2026-09-15-conv3d-claim-backpressure.md) | [同名原文](../../../推理框架/log-raw/2026-09-15-conv3d-claim-backpressure.md) | 120 | `sl-53e52b881e4d497da2c1bd65b270e8f3` |
...
<a id="plana-jd-w1-20260916-practice-handoff"></a>

#### `plana-jd-w1-20260916-practice-handoff`

- **日期与用户要求**：2026-09-16，开启 W1 最后两项实践的准备，后续在另一台机器开展；先记录开启并提供实践入口。
- **Lesson 引用**：`plana-jd-w1-vllm-execution-boundaries`。既有概念与材料验收保持有效，P1／P2 尚无本轮运行或验收结果，Lesson 仍为 `synthesis`。
- **接续工件**：新建 [W1 实践入口](../W1-实践入口.md)，说明两项的最小范围、目标机器只读核对项及实践约定确认方式；当前完成判断由 [W1](../W1.md)维护，精确恢复位置由[唯一学习断点](../../学习断点.md)维护。
- **待确认条件**：目标机器 OS、Python／vLLM 依赖、C++／Linux 工具链、固定源码可用性、隔离工作目录，以及所选最小用例的文件归属、环境操作与通过标准。此前会话机器和历史 Windows／GPU 环境均不能代替目标机器的核对。
- **状态与范围边界**：从暂缓转为实践准备已开启，不等于已接受正式实践契约；本次未安装依赖、下载资源、创建或执行测试，不修改参考源码或旧契约。不新增必做项目，不写入 `final_mastery`、工时或 W2 启动状态。

<a id="plana-jd-w1-20260916-p1-start"></a>

#### `plana-jd-w1-20260916-p1-start`

- **日期与上下文**：2026-09-16；`plana-jd-w1-vllm-execution-boundaries`。
- **实质动作**：用户明确接受 [P1 revision 1](#w1-p1-accepted-20260916)，digest 为 `sha256:d5bcc1c317f2c10155cc47d5667a4a2662daf72e7f215016f15d8414e0b81987`。导师完成工具基线与 CUDA 选择器核验；学习者完成路径源码问答、首次预测及纸面补差，并在步骤指导后提交核心实现。导师修复自身加载器后，同一原始实现主用例与保留 stop 的变式均 `3 passed`；变式行为预测正确，参数来源经补充讲解，P1 仍待学习者分析稿与解释验收。
- **证据边界**：[运行证据](../../../推理框架/实践/W1-P1/results.json)支持环境、配置选择及真实 OutputProcessor 的主用例行为。Core 输出由用例模拟，未证明实际 Core 取消、GPU KV 回收或 Attention kernel 推理；P2 未启动，未写入 final mastery 或未提供的工时。

<a id="plana-jd-w1-20260917-p1-analysis-review"></a>

#### `plana-jd-w1-20260917-p1-analysis-review`

- **日期与上下文**：2026-09-17；`plana-jd-w1-vllm-execution-boundaries` 的 P1 revision 1。
- **实质动作**：学习者提交并修订分析稿，导师核对源码和已保存证据。版本、类名、实际分支条件、证据层级与输出解释均通过；原 `p1-F4` 缩减为非阻塞定位笔误，按已接受约定完成 P1 的 A1–A4 验收。
- **Marker**：`practice-closed`，仅关闭 P1 练习。
- **证据边界**：学习者代码与验收工具哈希未变，复用两组 `3 passed`，未重复运行或代改学习者文件。保留指导与补差事实；未关闭 W1／Lesson、写入 final mastery、启动 P2 或记录未提供的时长。

<a id="plana-jd-w1-20260917-p2-start"></a>

#### `plana-jd-w1-20260917-p2-start`

- **日期与上下文**：2026-09-17；W1 附属 P2，复用已完成的 C++／Linux 概念验收。
- **实质动作**：用户接受 [P2 revision 1](#w1-p2-accepted-20260917)，digest 为 `sha256:6f073ac71a671e72c01706f8e8ad7743b998a8ba092c5b87fd12b3edf5b2fef5`。学习者完成正确预测、完整诊断、启动脚本与 B 父环境修复；随后正确预测 A 父环境变式，导师用同一脚本验证通过。运行阶段完成，进入分析稿交付。
- **证据边界**：只验证本次自建库与 A／B 两种父路径输入，尚待最终分析稿；未代写启动脚本。不扩大为全部 C++、RAII、并发、GPU 或性能能力，不关闭 W1，也不记录未提供的工时。

<a id="plana-jd-w1-20260918-p2-review-w1-gate"></a>

#### `plana-jd-w1-20260918-p2-review-w1-gate`

- **日期与上下文**：2026-09-18；W1 的 P2 分析稿终审及 W1 最终验收准备。
- **实质动作**：学习者提交分析稿，环境、诊断、修复与变式解释和证明边界经核对通过。脚本、工具与二进制哈希未变，复用已有运行证据；按接受的 P2 revision 1 完成 B1–B4 验收，汇总 [W1 最终验收提议](#w1-final-gate-proposal-20260918)。
- **Marker**：`practice-closed`，仅关闭 P2 练习；W1 置于 `mastery-gate`，等待用户确认。
- **证据边界**：未代改学习者文件、重复运行测试、写入 W1 final mastery、启动 W2 或记录未提供的工时。P1 的一个非阻塞路径笔误保留。

<a id="plana-jd-w1-20260918-practice-archive-cards"></a>

#### `plana-jd-w1-20260918-practice-archive-cards`

- **日期与用户要求**：2026-09-18，用户明确 P2 完结收口，但暂不确认 W1 完全完成；要求从发起 P1 实践的消息起提取 P1／P2 全过程记录，随后制卡，恢复点只保留 W1 最终收口。
- **学习状态**：P1／P2 的已有验收结论保持有效。W1 继续 `mastery-gate`，没有写入 final mastery，也没有启动 W2；本轮没有新增技术缺项或学习时长。
- **配对日志**：study-log 按主题导出下表，两组均为 `final`，包含过程更新、排除工具／推理／客户端注入与附件正文。P2 跨两份源文件，通过脚本分段归档后受控合并；30 条消息守恒，重发的终端输出保留独立消息 ID。原文隐私与代码保留范围已由用户确认，临时分段文件在配对校验后删除。

| 主题 | 结构化记录 | 可追溯可见文本 | 消息数／归档 ID |
|---|---|---|---|
| P1 运行路径与输出契约 | [学习记录](../../../推理框架/log/2026-09-18-w1-p1-runtime-output-practice.md) | [同名原文](../../../推理框架/log-raw/2026-09-18-w1-p1-runtime-output-practice.md) | 50／`sl-52d59b84044d4072b2cb440d64b70424` |
| P2 共享库诊断与启动修复 | [学习记录](../../../推理框架/log/2026-09-18-w1-p2-shared-library-practice.md) | [同名原文](../../../推理框架/log-raw/2026-09-18-w1-p2-shared-library-practice.md) | 30／`sl-b11920da801947d8b20e8f8fc37c8350` |

- **卡片交接**：memo-cards 仅消费两份结构化日志，新增 [P1 卡片](../../../推理框架/cards/w1-p1-runtime-output-practice.md) 8 张、[P2 卡片](../../../推理框架/cards/w1-p2-shared-library-practice.md) 4 张，生成两个受管 Markdown 与三个 XLSX（每个 4 张）。合计 8 张技术问答、4 张真实纠错；内容层级为 8 张原子、4 张机制。语义预筛复用 7 个旧主题，未修改旧卡；低频设备路径记忆暂缓，遗留问题与一次性环境标识不制卡，见[选择依据](../../../推理框架/cards/.requests/w1-practice-selection.json)。
- **校验**：两组 `verify-pairs` 通过；两个制卡请求的 `verify --request` 均返回 `operation=no-op`、`would_write=false`。模板、Markdown、XLSX、来源及 inventory 已核对。技能索引经中央 materializer 刷新并确认 current。
- **Git 与披露边界**：materializer 拒绝 intent-to-add 后，用户另行授权仅暂存上述两份结构化日志，已按精确路径执行。原文、卡片、实践文件和其他已有改动未暂存；没有 commit、push 或远端卡片上传。原文按用户确认保留本机路径、终端主机名、PID 与公开／自建练习代码，不称为匿名记录。

## 早期源码走读与验收摘录

以下是原设计包中的早期学习记录，保留当时题目、解释与边界；不替代当前 W1 完成判断。
...
- 结果型文章：[`深入学习理解vLLM/1-Repository-and-Process-Architecture.md`](../../../推理框架/深入学习理解vLLM/1-Repository-and-Process-Architecture.md)。
- 过程型记录：[`log/2026-08-09-vllm-repository-and-process-architecture.md`](../../../推理框架/log/2026-08-09-vllm-repository-and-process-architecture.md)。
- 技术记忆卡：[`cards/vllm-repository-and-process-architecture.md`](../../../推理框架/cards/vllm-repository-and-process-architecture.md)，共 25 张原子卡。
- 边界：以上材料只收录已通过的 Pass A–B；Pass C 的导师预讲不计入已掌握内容。


## 待实践约定（2026-09-15 快照）

以下是尚未启动的候选实践说明；当前是否完成只看 W1 的“还缺哪些没完成”，启动时仍需确认实际环境与约定。
...
- [请求生命周期与输出处理](../../../推理框架/深入学习理解vLLM/2-Request-Lifecycle-and-Output.md)。
...
以下只保留从原技术稿迁出的教学与验收背景，知识正文分别见[请求生命周期与输出处理](../../../推理框架/深入学习理解vLLM/2-Request-Lifecycle-and-Output.md)和[设备适配边界](../../../推理框架/深入学习理解vLLM/3-设备适配边界.md)。
