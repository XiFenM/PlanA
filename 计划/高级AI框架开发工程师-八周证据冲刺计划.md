...
### 19.4 市场观察与 W4 完成后的复核

按用户 2026-09-16 的约定，将[AMD 岗位与开源参与机会研究](AMD_LLM_Inference_Framework_JD_Research_2026-09-04.md)作为本专项的滚动参考：

- **文档职责**：每周汇总岗位要求、职级与投递定位、相关仓库变化和可参与的开源切口。文件名保留初版日期，采用时读取正文的研究日期、版本记录及核验范围；它不保存本计划的学习进度或掌握结论。
- **当前执行边界**：W4 按既有标准完成之前，不因研究文档更新而调整本计划的任务、周次、预算、验收标准或主读源码。文档中的四周准备路线、两周贡献路线及“本周优先”事项均只作研究建议，不替代或叠加现行八周安排，也不自动启动候选任务、投递或对外贡献。
- **复核触点**：既有 W4 完成后，再结合该文档当时的最新版本与 W1–W4 的实际验收证据，重新核对岗位状态、职级／地点条件、贡献候选的相关 PR／负责人／重复工作，以及可用硬件和最小可交付范围。这次对照不属于 W4 的新增验收项。
- **调整方式**：只有出现足以影响准备方向的新证据时，才提出岗位定位、W5–W8 案例或资料侧重点、W6 开源选题等小幅调整建议，由用户确认后执行；不自动增加总周数、投入预算或验收门槛，不把研究建议直接视为计划修订。
- **证据边界**：研究文档的个人匹配度估算与经历描述不替代项目档案、Claim-evidence 矩阵和真实验收；岗位开放、项目进展及 Issue／PR 状态只在其核验日期范围内有效，实际采用或投递前仍须重新核验一手来源。
