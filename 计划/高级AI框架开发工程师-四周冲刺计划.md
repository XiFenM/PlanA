...
- [ ] 先填写 §16.1 返回快照，再改写全局学习断点。
- [ ] 确认启动日期和可用周数。
- [ ] 确认工作日、周末和英语/Leetcode 是否沿用原节奏。
- [ ] 确认自研芯片、AMD、CUDA、CPU、多卡和 RDMA 哪些环境获准使用；不能使用时选择“公开源码审查”分支。
- [ ] 确认目标芯片可公开的能力槽位、SDK/文档可见性和公司保密边界；公开工件不得包含内部实现、未公开 API 或数据。
- [ ] 固定 vLLM、SGLang、MORI、PyTorch 的 tag/commit，明确 vLLM 主读、SGLang 窄对照、MORI 参考数据面的范围。
- [ ] 选择适配设计包的承载方式和脱敏规则；不要求创建独立代码仓库。
- [ ] 在当前 vLLM 为一张 Change Card 确定唯一上游验证锚点；实际编码预算上限为 8h，不作为启动前提。
- [ ] 建立源码、能力矩阵、Change Card、风险和验证证据的记录位置。
- [ ] 不预先启动 `mini-ep-pd-serving`；只有核心周验收提前通过后再决定是否选做一个切片。
- [ ] 通过陪学流程把断点切换到本专项的第一个实际任务。
